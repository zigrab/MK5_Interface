...
<fieldset>
  <legend>Join A Network<help id='network:join_network'></help></legend>
  <p>
    Interface: <select name="iface">
      <?php 
      exec("iwconfig 2>&1 | grep 'IEE' | awk '{print $1}'", $interfaces);
      foreach ($interfaces as $interface) {
        if (strpos($interface, 'wlan0') === false && strpos($interface, 'mon') === false) {
...
<fieldset>
  <legend>Connection Information - <a href="JAVASCRIPT:check_connection()">Refresh</a> - <a href="JAVASCRIPT:disconnect()">Disconnect Selected Interface</a></legend>
    Interface: <select name="connected_iface">
      <?php 
      foreach ($interfaces as $interface) {
        if (strpos($interface, 'wlan0') === false && strpos($interface, 'mon') === false) echo "<option value='".str_replace('wlan', '', $interface)."'>$interface</option>";
      }
      ?>
    </select>  
  <p>
