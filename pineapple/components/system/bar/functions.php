...
if (isset($_GET['action'])) {
    switch ($_GET['action']) {
        ...
        case 'getInfusionList':
            ...
            break;
        case 'save':
            echo save_edited_infusion($_POST['name'], $_POST['handler'], $_POST['small_tile'], $_POST['large_tile'], $_POST['functions']);
            break;
    }
}
...
function createNewInfusion($name, $title, $type, $version, $external = 'false')
{
    if (is_numeric($version)) {
        $version = ((int) $version != $version) ? $version : $version . ".0";
    } else {
        $version = "1.0";
    }
    ...
    if (!file_exists('/pineapple/components/infusions/'.$name)) {
        ...
        if ($created) {
            ...
            if ($type == 0 || $type == 1) {
                ...
                exec("touch /pineapple/components/infusions/$name/help.json");
...
function getInfusionList()
{
    if (online()) {
        ...
        if (sizeof($infusions->sys)) {
            $html .= "<table style='border-spacing: 15px'>
        ...
        $html .= "</fieldset><br />
        <fieldset>
        <legend>User Infusions<help id='bar:usr_infusions'></help> - <a id='infusion_toggle' onclick='infusion_toggle()' href='#'>Show</a></legend>
        ...
        $html .= "<fieldset><legend>Command Line Infusions<help id='bar:cli_infusions'></help> - <a id='cli_infusion_toggle' onclick='cli_infusion_toggle()' href='#'>Show</a></legend>";
        if (sizeof($infusions->cli)) {

            $html .= "<table style='border-spacing: 15px; display:none;' id='available_cli_infusions'>
            <th>Name</th><th>Version</th><th>Description</th><th>Author</th><th>Size</th>";
...
function download_status($destination, $infusion_string)
{
    ...
    if ($percentage == 100) {
        if ($destination == 'internal') {
            if (!file_exists('/tmp/infusions/downloading')) {
                return 'completed';
            }
        } else {
            if (!file_exists('/sd/tmp/infusions/downloading')) {
                return 'completed';
            }
        }
    }
...
function edit_infusion($infusion)
{
    ...
    if (file_exists($path."handler.php")) {
        ...
        $handler_code = file_get_contents($path."handler.php");
        ...
        <form method='POST' action='".$rel_dir."functions.php?action=save' id='edit_form' onSubmit='\$(this).AJAXifyForm(edit_infusion_form); return false;'>
            ...
            <fieldset>
                <legend><b>Infusion Handler</b> (/pineapple/components/infusions/{$infusion}/handler.php)</legend>
                <textarea name='handler' style='width: 100%; height: 40em'>{$handler_code}</textarea>
            </fieldset>
            <br /><br />
            <fieldset>
                <legend><b>Small Tile Code</b> (/pineapple/components/infusions/{$infusion}/small_tile.php)</legend>
                <textarea name='small_tile' style='width: 100%; height: 40em'>{$small_tile_code}</textarea>
            </fieldset>
            <br /><br />
            <fieldset>
                <legend><b>Large Tile Code</b> (/pineapple/components/infusions/{$infusion}/large_tile.php)</legend>
                <textarea name='large_tile' style='width: 100%; height: 40em'>{$large_tile_code}</textarea>
            </fieldset>
            <br /><br />
            <fieldset>
                <legend><b>Functions Code</b> (/pineapple/components/infusions/{$infusion}/functions.php)</legend>
...
function save_edited_infusion($infusion, $handler, $small_tile, $large_tile, $functions)
{
    $path = "/pineapple/components/infusions/$infusion/";
    file_put_contents($path . 'handler.php', str_replace("\r", "", $handler));
