...
<center>
    <div id='messages' />
    <h2>New Infusion<help id='bar:new_infusion'></help></h2>
    <form method='POST' action='<?=$rel_dir?>functions.php?action=create' id='create_form' onSubmit='$(this).AJAXifyForm(create_infusion); return false;'>
        <table>
            ...
            <?php
            if (sd_available()) {
                echo "<tr><td>Create on SD?</td><td><select name='external'><option value='false'>false</option><option value='true'>true</option></select></td></tr>";
            }
            ?>
        </table>
